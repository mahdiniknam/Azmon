        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div class="mt-4 md:mt-0">
                <a href="{{ route('teacher.reports.index') }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg bg-gray-600 text-white hover:bg-gray-500 shadow-sm">بازگشت</a>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-start">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($participants as $participant)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ $participant->user->email ?? ($participant->user->phone ?? '-') }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ $participant->started_at ? jdate($participant->started_at)->format('Y/m/d H:i') : '-' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    @if ($participant->invalidated_at)
                                        ...
                                        <div class="text-gray-500 text-xs">
                                            دلیل: {{ $participant->invalidated_reason }}
                                        </div>
                                    @elseif (($participant->suspicious_events_count ?? 0) > 0)
                                        <form action="{{ route('teacher.reports.invalidate-attempt', $participant) }}"
                                            method="POST" class="space-y-2">
                                            @csrf
                                            <textarea name="reason" rows="2" class="w-full rounded-lg border border-gray-300 text-sm"
                                                placeholder="دلیل تقلب را بنویسید..." required></textarea>

                                            <button type="submit"
                                                onclick="return confirm('آیا مطمئن هستید که می‌خواهید نمره این دانشجو را صفر کنید؟')"
                                                class="px-3 py-2 bg-red-600 text-white text-xs rounded-lg hover:bg-red-700">
                                                ثبت تقلب و صفر کردن نمره
                                            </button>
                                        </form>
                                    @else
                                        <span class="text-gray-400 text-xs">مورد مشکوکی ثبت نشده</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center">شرکت کننده‌ای یافت نشد.</td>
                            </tr>
                        @endforelse
