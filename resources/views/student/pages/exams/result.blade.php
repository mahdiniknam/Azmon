...
@section('student-content')
    <main class="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
        ...
        @if (isset($resultPublished) && !$resultPublished)
            <div
                class="bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 p-6 rounded-xl border border-yellow-200 dark:border-yellow-800 text-center">
                ...
                <div class="mt-6">
                    <a href="{{ route('student.exams.index') }}"
                        class="inline-flex items-center rounded-lg bg-yellow-600 px-4 py-2 text-white hover:bg-yellow-700">بازگشت
                        به لیست آزمون‌ها</a>
                </div>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md border border-gray-200 dark:border-gray-700 text-center">
            ...
            @if (isset($subjectBreakdown) && count($subjectBreakdown) > 0)
                <div
                    class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 mb-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-start">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">درس
                                    </th>
                                    <th
                                        class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase text-center">
                                        صحیح</th>
                                    <th
                                        class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase text-center">
                                        غلط</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach ($subjectBreakdown as $subject)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-green-500">
                                            {{ $subject['correct'] }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                                            {{ $subject['unanswered'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center font-bold" dir="ltr">
                                            {{ $subject['percentage'] }}%</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            @endif
            @if ($attempt->invalidated_at)
                <div class="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">
                    <div class="font-bold mb-2">این آزمون به دلیل تقلب مردود شده است.</div>
                    <div class="text-sm">
                        دلیل ثبت‌شده توسط استاد:
                        <span class="font-medium">{{ $attempt->invalidated_reason }}</span>
                    </div>
                </div>
            @endif
            ...
            <div class="mt-6 flex justify-end">
                <a href="{{ route('student.exams.index') }}"
                    class="inline-flex items-center rounded-lg bg-gray-600 px-6 py-2 text-white hover:bg-gray-700 font-medium">بازگشت
                    به لیست آزمون‌ها</a>
            </div>
        @endif
    </main>
