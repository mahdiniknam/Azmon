        ...
        <form action="{{ route('admin.questions.update',$question->id) }}" method="POST"
            ...
            <div class="px-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="subject_id" class="select2 w-full border rounded-lg px-3 py-2">
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}" @selected($subject->id == $question->subject->id)>
                                {{ $subject->title }}
                            </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input type="number" step="0.25" name="score" value="{{ old('score', $question->score) }}"
                ...
                <div>
                    ...
                    <select name="difficulty" class="w-full border rounded-lg px-3 py-2">
                        ...
                        @foreach (\App\Models\Question::difficultys() as $key => $label)
                            <option value="{{ $key }}" @selected($key == $question->difficulty)>{{ $label }}</option>
                        @endforeach
            ...
            <div class="px-6">
                ...
                <textarea rows="5" name="question_text" class="w-full border rounded-lg px-3 py-2">{{ old('question_text', $question->question_text) }}</textarea>
                ...
                @foreach ($question->options as $index => $option)
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <input type="text" name="options[{{ $option->id }}]"
                            ...
                            placeholder="گزینه {{ $index + 1 }}" class="flex-1 border rounded-lg px-3 py-2">
