...
<main class="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">ویرایش درس </h2>
            <p class="text-gray-600 dark:text-gray-400">اطلاعات درس  را در فرم زیر وارد کنید</p>
        </div>
    ...
    <!-- Add user form -->
    <form action="{{ route('admin.subjects.update',$subject->id) }}" method="post" class="bg-white py-6 space-y-4 dark:bg-gray-800 rounded-xl shadow-md dark:shadow-gray-700 overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="status" class="select2 w-full focus:border-primary border-gray-300 outline-0 px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-primary-500 @error('countryId') is-invalid @enderror">
                        ...
                        <option value="{{ $key }}" @selected($key==$subject->status)>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('status')
                    <x-alert type="error" message="{{ $message }}"></x-alert>
