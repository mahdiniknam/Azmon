...
@section('admin-title')
@lang('general.Arya Gostaran Management Panel') / @lang('general.departments_list')
@endsection
...
<main class="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">@lang('general.departments_management')</h2>
            <p class="text-gray-600 dark:text-gray-400">@lang('general.departments_list')</p>
        </div>
        ...
        <div class="mt-4 md:mt-0">
            <a href="{{ route('admin.departments.create') }}"
               class="flex items-center px-4 py-2 text-sm font-medium rounded-lg bg-primary-600 text-white hover:bg-primary-500 shadow-sm dark:bg-primary-500 dark:hover:bg-primary-600">
                ...
                @lang('general.create_department')
            </a>
        </div>
    </div>
            ...
            <table class="w-full text-start">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">@lang('general.name')</th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">@lang('general.description')</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    @forelse($departments as $department)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{{ $department->id }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">@lang('general.disabled')</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-4">
                                    ...
                                    <form action="{{ route('admin.departments.destroy', $department) }}" method="POST"
                                          onsubmit="return confirm('@lang('general.are_you_sure')')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
